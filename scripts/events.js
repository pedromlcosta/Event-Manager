...
function invite(event) {
	...
	var user = $(".overlayUser").val();
	$(".overlayUser").val('');

    $.post("action_invite.php",
    {
        user:user,
        eventID:eventID,
        userID:userID
...
function displayData(myData){
	$("#overlay #errorFeedback").text(myData['val']);
	if(myData['user']){
    var users = $("#overlay #usersInvited #users").text();
    if(users.length != 0)
    	users += ", ";
    users += myData['user'];
	$("#overlay #usersInvited #users").text(users);
	}
}
...
function closeInviting(){
	...
	$("#overlay #usersInvited #users").text('');
}
